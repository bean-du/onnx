...
use ort::execution_providers::{CoreMLExecutionProviderOptions, CUDAExecutionProviderOptions, OpenVINOExecutionProviderOptions, TensorRTExecutionProviderOptions};
...
impl OrtBackend {
    pub fn build(args: OrtConfig) -> Result<Self> {
        ...
        let (ep, provider) = match args.ep {
            ...
            OrtEP::OpenVino => Self::set_ep_openvino(),
            OrtEP::CoreML => Self::set_ep_core_ml(),
    ...
    pub fn set_ep_core_ml() -> (OrtEP, ExecutionProvider) {
        // set CoreML
        if ExecutionProvider::CoreML(Default::default()).is_available() {
            (
                OrtEP::CoreML,
            ...
            info!("{}","> CoreML is not available! Using CPU.");
            (OrtEP::Cpu, ExecutionProvider::CPU(Default::default()))
        }
    }

    pub fn set_ep_openvino() -> (OrtEP, ExecutionProvider) {
        // set OpenVINO
        if ExecutionProvider::OpenVINO(OpenVINOExecutionProviderOptions {
            enable_opencl_throttling: true,
            ..Default::default()
        }).is_available() {
            (
                OrtEP::OpenVino,
                ExecutionProvider::OpenVINO(OpenVINOExecutionProviderOptions {
                    enable_opencl_throttling: true,
                    ..Default::default()
                }),
            )
        } else {
            info!("{}","> OpenVINO is not available! Using CPU.");
            (OrtEP::Cpu, ExecutionProvider::CPU(Default::default()))
        }
    }
